...
const filterData = [
  ...
  },
  {
    ...
    id: "experience",
    array: ["Fresher (0-1 yr)", "0-3 years", "3-5 years", "5-7 years", "7-10 years", "10+ years"],
  },
  {
    ...
    <div className="w-full h-full bg-white flex flex-col">
      <div className="flex items-center justify-between pb-4 border-b border-slate-100">
        <div className="flex items-center gap-2">
          <h1 className="font-black text-lg text-slate-900">Filters</h1>
          {activeFilterCount > 0 && (
        ...
        {activeFilterCount > 0 && (
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-semibold text-rose-500 hover:text-rose-600 bg-rose-50 hover:bg-rose-100 px-2.5 py-1.5 rounded-lg transition-colors"
          >
      ...
      <div className="mt-4 flex-1 overflow-y-auto pr-1 space-y-5">
        {filterData.map((data) => (
          <div key={data.id} className="border-b border-slate-100 pb-4 last:border-0 last:pb-0">
            <button
              onClick={() => toggleSection(data.id)}
              className="flex items-center justify-between w-full group mb-1"
            >
              <div className="flex items-center gap-2">
                <h2 className="font-bold text-[14px] text-slate-800 group-hover:text-blue-600 transition-colors">
                  {data.filterType}
                </h2>
              ...
              <ChevronDown
                className={`h-4 w-4 text-slate-400 transition-transform duration-200 ${expandedSections[data.id] ? "rotate-180" : ""}`}
              />
            </button>
                ...
                >
                  <div className="mt-3 space-y-2">
                    {data.id === "location" && (
                      <div className="mb-3 relative">
                        <input
                          ...
                          className="w-full text-xs rounded-lg border border-slate-200 py-2 pl-3 pr-8 focus:outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-400"
                    ...
                    {data.id === "location" && selectedFilters.location?.filter(loc => !data.array.includes(loc)).map((loc, idx) => (
                      <label key={`custom-${idx}`} className="flex items-center group cursor-pointer">
                        ...
                        <span className="ml-2.5 text-sm font-semibold text-slate-900">{loc}</span>
                      </label>
                    ))}
                    {data.array.map((item, idx) => {
                      ...
                      return (
                        <label key={idx} className="flex items-center group cursor-pointer">
                          ...
                          />
                          <div className={`flex h-4.5 w-4.5 h-[18px] w-[18px] items-center justify-center rounded-md border flex-shrink-0 transition-colors ${isChecked ? "border-blue-600 bg-blue-600" : "border-slate-300 bg-white group-hover:border-blue-400"}`}>
                            {isChecked && <Check className="h-2.5 w-2.5 text-white" />}
                          </div>
                          <span className={`ml-2.5 text-sm ${isChecked ? "font-semibold text-slate-900" : "text-slate-600 group-hover:text-slate-900"}`}>
                            {item}
                          </span>
                        </label>
                      );
