...
const TypingBubble = () => (
  <div className="flex justify-start">
    <div className="flex items-center gap-1 rounded-2xl rounded-bl-sm bg-white dark:bg-slate-800 px-4 py-3 shadow-sm border border-slate-100 dark:border-slate-800/80">
      {[0, 1, 2].map((i) => (
        <motion.span key={i} className="h-2 w-2 rounded-full bg-slate-400 dark:bg-slate-500"
...
const ChatPanel = ({
  ...
  /* ─────────────────────────────── RENDER ─────────────────────────────────── */
  return (
    <div className="flex flex-1 min-h-0 w-full overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xl">

      {/* ═══════════════ LEFT: Conversation List ═══════════════════════════ */}
      <div className={`flex-col border-r border-slate-100 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-950/40 w-full sm:w-80 sm:flex-shrink-0 ${mobileView === "list" ? "flex" : "hidden sm:flex"}`}>
        <div className="flex flex-shrink-0 items-center justify-between border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-3.5 shadow-sm">
          <div>
            <h2 className="text-sm font-bold text-slate-900 dark:text-white">Conversations</h2>
            {!connected && (
              <span className="flex items-center gap-1 text-[10px] font-semibold text-amber-600 dark:text-amber-500">
        ...
        {user?.role === "Recruiter" && (
          <div className="flex border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900/50 p-2 gap-1.5 flex-shrink-0">
            <button
              type="button"
          ...
          {loadingConvos ? (
            ...
            filteredConversations.map((c) => (
              <button key={c._id}
                ...
                className={`flex w-full items-start gap-3 border-b border-slate-100 dark:border-slate-800 px-4 py-3.5 text-left transition-all duration-150 ${selectedId === c._id ? "border-l-[3px] border-l-blue-500 bg-blue-50/60 dark:bg-slate-800/40" : "hover:bg-white dark:hover:bg-slate-900/40"}`}>
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-1">
                    <p className={`truncate text-sm ${c.unreadCount > 0 ? "font-bold text-slate-900 dark:text-white" : "font-semibold text-slate-700 dark:text-slate-300"}`}>
                  ...
                  {c.jobTitle && (
                    <p className="mb-0.5 flex items-center gap-1 truncate text-[11px] font-semibold text-blue-600 dark:text-blue-400">
                      <Briefcase className="h-2.5 w-2.5 flex-shrink-0" />{c.jobTitle}
                    </p>
                  )}
                  <p className={`truncate text-xs ${c.unreadCount > 0 ? "font-semibold text-slate-700 dark:text-slate-300" : "text-slate-400 dark:text-slate-500"}`}>
      ...
      <div className={`flex-col flex-1 min-w-0 ${mobileView === "thread" ? "flex" : "hidden sm:flex"}`}>
        {!selected ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-4 bg-slate-50/50 dark:bg-slate-950/40">
            <div className="flex h-20 w-20 items-center justify-center rounded-3xl bg-gradient-to-br from-blue-100 to-cyan-100 dark:from-slate-800 dark:to-slate-900">
              <MessageCircle className="h-10 w-10 text-blue-400 dark:text-blue-500" />
            </div>
            <div className="text-center px-4">
              <p className="text-base font-bold text-slate-600 dark:text-slate-400">No conversation selected</p>
              <p className="mt-1 max-w-xs text-xs text-slate-400 dark:text-slate-500">{emptyStateLabel}</p>
            </div>
          </div>
        ) : (
          <>
            {/* Thread header */}
            <div className="flex flex-shrink-0 items-center justify-between border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-3 shadow-sm">
              <div className="flex min-w-0 items-center gap-3">
                <button onClick={() => setMobileView("list")}
                  className="mr-1 flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 sm:hidden">
                  <ArrowLeft className="h-5 w-5 text-slate-600 dark:text-slate-300" />
                </button>
                <Avatar photo={selected.otherParticipant.profilePhoto} name={selected.otherParticipant.fullname} size="sm" />
                <div className="min-w-0">
                  <p className="truncate text-sm font-bold text-slate-900 dark:text-white">{selected.otherParticipant.fullname}</p>
                    ...
                    {renderItems.map((item) => {
                      if (item.type === "date") {
                        return (
                          <div key={item.key} className="my-3 flex items-center justify-center">
                            <span className="rounded-full bg-slate-200/80 dark:bg-slate-800 px-4 py-1 text-[10px] font-bold text-slate-500 dark:text-slate-400">{item.label}</span>
                          </div>
                        );
                      }
                      const m = item.data;
                      ...
                      return (
                        <motion.div key={item.key}
                          ...
                          <div className={`max-w-[70%] sm:max-w-[60%] rounded-2xl px-4 py-2.5 text-sm shadow-sm ${mine ? "rounded-br-sm bg-gradient-to-br from-blue-600 to-cyan-600 text-white" : "rounded-bl-sm bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-250 border border-slate-100 dark:border-slate-700/60"}`}>
                            ...
                            {m.attachmentUrl && m.attachmentType !== "image" && (
                              <a href={getFileUrl(m.attachmentUrl)} target="_blank" rel="noreferrer" download={m.attachmentName || "file"} onClick={(e) => e.stopPropagation()}
                                className={`mb-1.5 flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold ${mine ? "bg-white/20 text-white" : "bg-slate-100 dark:bg-slate-900 text-slate-750 dark:text-slate-300"}`}>
            ...
            {/* Input */}
            <div className="flex-shrink-0 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-3">
              {selected.status === "pending" ? (
                <div className="flex items-center justify-center gap-2 rounded-xl bg-amber-50 dark:bg-amber-950/20 py-3 text-xs font-semibold text-amber-700 dark:text-amber-500">
                  <Clock className="h-4 w-4" />
                  {selected.iInitiated ? "Waiting for the recruiter to accept your request" : "Accept the request above to start chatting"}
                </div>
              ) : selected.status === "rejected" ? (
                <div className="flex items-center justify-center gap-2 rounded-xl bg-rose-50 dark:bg-rose-950/20 py-3 text-xs font-semibold text-rose-500 dark:text-rose-450">
                  <X className="h-4 w-4" /> This chat request was declined
                </div>
              ) : (
                <div className="flex flex-col gap-2">
                  {attachment && (
                    <div className="flex w-fit items-center gap-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-3 py-2">
                      {attachment.isImage ? (
                        <img src={attachment.previewUrl} className="h-10 w-10 rounded-lg object-cover" alt="preview" />
                      ) : (
                        <FileText className="h-5 w-5 text-slate-400" />
                      )}
                      <span className="max-w-[160px] truncate text-xs font-semibold text-slate-600 dark:text-slate-400">{attachment.file.name}</span>
                  ...
                  <div className="flex items-end gap-2">
                    ...
                    <button onClick={() => fileInputRef.current?.click()}
                      className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-400 transition-colors hover:border-blue-300 dark:hover:border-blue-750 hover:bg-blue-50 dark:hover:bg-slate-800 hover:text-blue-600 dark:hover:text-blue-400"
                      title="Attach file">
                    ...
                    <textarea ref={textareaRef} rows={1} value={input} onChange={handleInputChange}
                      ...
                      className="max-h-[120px] flex-1 resize-none rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-4 py-2.5 text-sm transition-colors focus:border-blue-400 dark:focus:border-blue-600 focus:bg-white dark:focus:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900/30 text-slate-900 dark:text-white scrollbar-none" />
