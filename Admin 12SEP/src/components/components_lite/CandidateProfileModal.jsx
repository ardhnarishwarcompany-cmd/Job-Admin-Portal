    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm">
        <motion.div
          ...
          className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 md:p-8 shadow-2xl transition-all duration-300"
        >
          {/* Close button */}
          <button
            onClick={onClose}
            className="absolute top-4 right-4 rounded-xl bg-slate-100 dark:bg-slate-800 p-2 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-350 transition-colors"
          >
            <X className="h-5 w-5" />
          </button>

          {/* Header Row */}
          <div className="flex flex-col md:flex-row gap-6 items-start md:items-center border-b border-slate-100 dark:border-slate-800 pb-6 mb-6">
            <img
              src={profilePhoto}
              alt={user.fullname}
              className="h-24 w-24 rounded-2xl object-cover border-4 border-slate-100 dark:border-slate-800 shadow-md"
            />
            <div className="flex-1">
              <div className="flex flex-wrap items-center gap-3">
                <h3 className="text-2xl font-black text-slate-900 dark:text-white">{user.fullname}</h3>
                <span className="rounded-full bg-blue-100 dark:bg-blue-950 px-3 py-1 text-xs font-black text-blue-700 dark:text-blue-400 uppercase tracking-wider">Candidate</span>
              </div>
              <p className="text-sm text-blue-600 dark:text-blue-400 font-bold mt-1">
                {career.jobTitle || "Seeking career opportunities"}
              </p>
              <div className="flex flex-wrap gap-4 text-xs text-slate-500 dark:text-slate-400 mt-3 font-semibold">
                <span className="flex items-center gap-1"><Mail className="h-3.5 w-3.5" />{user.email}</span>
                <span className="flex items-center gap-1"><Phone className="h-3.5 w-3.5" />{user.phoneNumber || "N/A"}</span>
                <span className="flex items-center gap-1"><MapPin className="h-3.5 w-3.5" />{personal.city ? `${personal.city}, ${personal.country}` : "Location not set"}</span>
              </div>
            </div>
            
            {/* Completion Gauge */}
            <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-950/60 border border-slate-100 dark:border-slate-800 p-4 rounded-2xl">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/20 p-5">
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  ...
                  <div>
                    ...
                    <span className="font-semibold text-slate-850 dark:text-slate-200 mt-0.5 block">{personal.dob || "N/A"}</span>
                  </div>
                  <div>
                  ...
                  <div className="col-span-full">
                    ...
                    <span className="font-semibold text-slate-855 dark:text-slate-200 mt-0.5 block leading-relaxed">
                ...
                <div className="grid grid-cols-2 gap-3.5 text-xs">
                  ...
                  <div className="col-span-full">
                    ...
                    <div className="flex flex-wrap gap-2 mt-1">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${workPrefs.relocate ? "bg-blue-100 text-blue-700 dark:bg-blue-950/50 dark:text-blue-400" : "bg-slate-100 text-slate-450 dark:bg-slate-805/40"}`}>Ready to Relocate</span>
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${workPrefs.travel ? "bg-blue-100 text-blue-700 dark:bg-blue-950/50 dark:text-blue-400" : "bg-slate-100 text-slate-450 dark:bg-slate-805/40"}`}>Ready to Travel</span>
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${workPrefs.passport ? "bg-blue-100 text-blue-700 dark:bg-blue-950/50 dark:text-blue-400" : "bg-slate-100 text-slate-455 dark:bg-slate-805/40"}`}>Passport Ready</span>
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${workPrefs.ownVehicle ? "bg-blue-100 text-blue-700 dark:bg-blue-950/50 dark:text-blue-400" : "bg-slate-100 text-slate-455 dark:bg-slate-805/40"}`}>Own Vehicle</span>
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${workPrefs.drivingLicense ? "bg-blue-100 text-blue-700 dark:bg-blue-950/50 dark:text-blue-400" : "bg-slate-100 text-slate-455 dark:bg-slate-805/40"}`}>Driving License</span>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/20 p-5">
                  ...
                  <div className="space-y-3.5 text-xs">
                    {certifications.map((cert, index) => (
                      <div key={index} className="border-b border-slate-150 pb-2.5 last:border-b-0 last:pb-0">
                        <p className="font-bold text-slate-850 dark:text-slate-200 text-xs">{cert.name}</p>
                        <p className="text-[10px] text-slate-450 mt-0.5 font-bold">{cert.issuer} • Issued: {cert.issueDate || "N/A"} {cert.expiryDate ? `• Exp: ${cert.expiryDate}` : ""}</p>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="mt-6 rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/20 p-5">
            <h4 className="font-bold text-slate-900 dark:text-white text-sm mb-3.5 flex items-center gap-2">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs font-bold">
              {portfolio.linkedin ? (
                <a href={portfolio.linkedin} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-slate-700 dark:text-slate-350 hover:text-blue-600">
                  <Linkedin className="h-4 w-4 text-[#0077b5]" /> LinkedIn
                </a>
              ) : <span className="text-slate-400 dark:text-slate-600 flex items-center gap-2"><Linkedin className="h-4 w-4" /> LinkedIn (N/A)</span>}

              {portfolio.github ? (
                <a href={portfolio.github} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-slate-700 dark:text-slate-350 hover:text-slate-950 dark:hover:text-white">
                  <Github className="h-4 w-4 text-slate-800 dark:text-slate-200" /> GitHub
                </a>
              ) : <span className="text-slate-400 dark:text-slate-600 flex items-center gap-2"><Github className="h-4 w-4" /> GitHub (N/A)</span>}

              {portfolio.portfolioWebsite ? (
                <a href={portfolio.portfolioWebsite} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-slate-700 dark:text-slate-350 hover:text-blue-600">
                  <Globe className="h-4 w-4 text-cyan-500" /> Portfolio Website
                </a>
              ) : <span className="text-slate-400 dark:text-slate-600 flex items-center gap-2"><Globe className="h-4 w-4" /> Portfolio (N/A)</span>}

              {portfolio.dribbble ? (
                <a href={portfolio.dribbble} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-slate-700 dark:text-slate-350 hover:text-pink-600">
                  <Heart className="h-4 w-4 text-pink-500" /> Dribbble
                </a>
              ) : <span className="text-slate-400 dark:text-slate-600 flex items-center gap-2"><Heart className="h-4 w-4" /> Dribbble (N/A)</span>}
            </div>
          </div>
