...
import { AlertCircle, CheckCircle2, FileText, Lightbulb, Loader2, Sparkles, UploadCloud } from "lucide-react";
import Navbar from "./Navbar";
import { Button } from "../ui/button";
import { parseResumePdf, getAtsScore } from "@/utils/aiResumeTools.js";
...
const ResumeScanner = () => {
  ...
  const handleFileUpload = async (event) => {
    ...
    if (extension === "pdf") {
      setUploading(true);
      setFileNotice("Extracting text from your PDF...");
      try {
        const data = await parseResumePdf(file);
        if (data.success) {
          setResumeText(data.text);
          setFileNotice(`Extracted ${data.pages ? `${data.pages} page(s)` : "text"} from your PDF successfully.`);
        } else {
          setFileNotice(data.message || "Could not extract text from this PDF. Paste resume text below instead.");
        }
      } catch (err) {
        setFileNotice(err?.response?.data?.message || "Could not extract text from this PDF. Paste resume text below instead.");
      } finally {
        setUploading(false);
      }
      return;
    ...
    setFileNotice("DOC/DOCX upload accepted, but text extraction here works best with PDF or TXT. Paste resume text below for the most accurate score.");
  };

  const runAiScore = async () => {
    if (!resumeText.trim()) {
      toast.error("Add resume text or upload a resume first.");
      return;
    }
    setAiScoring(true);
    setAiError("");
    try {
      ...
      if (data.success) {
        ...
        setAiError(data.message || "Could not run AI analysis right now.");
      }
    } catch (err) {
      setAiError(err?.response?.data?.message || "Could not run AI analysis right now. Please try again.");
    } finally {
      setAiScoring(false);
    }
  };
        ...
        <section className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
          <div className="space-y-4">
            <label className="flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-teal-300 bg-white px-6 py-8 text-center shadow-sm hover:border-teal-500 hover:bg-teal-50">
              {uploading ? (
                <Loader2 className="mb-3 h-10 w-10 animate-spin text-teal-700" />
              ) : (
                <UploadCloud className="mb-3 h-10 w-10 text-teal-700" />
              )}
              <span className="text-base font-black text-slate-900">{uploading ? "Reading PDF..." : "Upload resume"}</span>
              <span className="mt-1 text-sm text-slate-500">PDF or TXT works best — we extract the text automatically.</span>
              <input type="file" accept=".txt,.md,.csv,.pdf,.doc,.docx" onChange={handleFileUpload} className="sr-only" disabled={uploading} />
            </label>
            ...
            <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
              <div className="mb-3 flex items-center justify-between gap-3">
                <h2 className="flex items-center gap-2 text-xl font-black text-slate-950">
                  ...
                  AI-verified ATS analysis
                </h2>
                <Button type="button" onClick={runAiScore} disabled={aiScoring} className="h-8 bg-teal-700 text-xs hover:bg-teal-800">
                  {aiScoring ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Sparkles className="h-3.5 w-3.5" />}
                  <span className="ml-1.5">{aiScoring ? "Analyzing..." : "Get AI Score"}</span>
                </Button>
              </div>

              {!aiScoring && !aiError && !aiResult && (
                <p className="text-sm text-slate-500">
                  The score above is instant, local, and rule-based. Click "Get AI Score" for a deeper, AI-generated review of this resume's content.
                </p>
              )}
              ...
              {!aiScoring && aiError && (
                <div className="flex items-start gap-2 rounded-md bg-red-50 p-3 text-sm text-red-700">
                  <AlertCircle className="mt-0.5 h-4 w-4 flex-none" /> {aiError}
                </div>
              )}
              ...
              {!aiScoring && aiResult && (
                <div className="space-y-3">
                  <div className="flex items-center gap-4">
                    <div className="relative h-20 w-20 flex-none rounded-full bg-slate-100">
                      <div
                        className="absolute inset-0 rounded-full"
                        style={{ background: `conic-gradient(#0f766e ${aiResult.score * 3.6}deg, #e2e8f0 0deg)` }}
                      />
                      <div className="absolute inset-2.5 grid place-items-center rounded-full bg-white text-sm font-black text-teal-700">
                        {aiResult.score}%
                      </div>
                    </div>
                    <div>
                      <p className="text-sm font-black text-slate-900">{aiResult.verdict}</p>
                      <p className="mt-1 text-xs text-slate-600">{aiResult.summary}</p>
                    </div>
                  </div>
                  {aiResult.improvements?.length > 0 && (
                    <div className="space-y-2">
                      {aiResult.improvements.map((item, i) => (
                        <div key={i} className="flex gap-2 rounded-md bg-amber-50 p-2.5 text-xs font-medium text-slate-700">
