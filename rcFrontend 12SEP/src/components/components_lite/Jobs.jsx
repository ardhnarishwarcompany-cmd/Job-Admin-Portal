...
const Jobs = () => {
  ...
  const activeJobs = React.useMemo(() => (allJobs || []).filter((j) => !j.isDeleted && j.status !== "removed"), [allJobs]);
  const [filterJobs, setFilterJobs] = useState(activeJobs);
  ...
  useEffect(() => {
    if (!searchedQuery || typeof searchedQuery !== "object") {
      ...
      if (!q) { setFilterJobs(activeJobs); return; }
      setFilterJobs(
  ...
  }, [activeJobs, searchedQuery]);

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-cyan-50/10 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
      <Navbar />
      ...
      <motion.div
        ...
        className="border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-4 shadow-sm"
      >
        <div className="mx-auto max-w-7xl flex items-center justify-between">
          <div>
            <h1 className="text-xl font-black text-slate-900 dark:text-white">
              <span className="text-blue-600 dark:text-blue-400">{filterJobs.length}</span> Jobs Available
            </h1>
            {searchedQuery && typeof searchedQuery === "string" && (
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
                Results for "<span className="font-semibold text-blue-600 dark:text-blue-400">{searchedQuery}</span>"
              </p>
            )}
          </div>
          <button
            onClick={() => setMobileFilterOpen(true)}
            className="flex items-center gap-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-2 text-sm font-bold text-slate-700 dark:text-slate-300 hover:border-blue-300 dark:hover:border-blue-700 hover:text-blue-700 dark:hover:text-blue-400 transition-colors lg:hidden"
          >
      ...
      <div className="mx-auto max-w-7xl px-4 py-6">
        <div className="flex gap-6">
          ...
          >
            <div className="sticky top-20 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-md overflow-y-auto max-h-[calc(100vh-90px)]">
              <CvJobSearch />
              <FilterCard />
            </div>
          </motion.div>
          ...
          {mobileFilterOpen && (
            ...
            >
              <motion.div
                ...
                className="absolute left-0 top-0 bottom-0 w-80 bg-white dark:bg-slate-900 p-4 shadow-2xl overflow-y-auto border-r border-slate-200 dark:border-slate-800"
              >
                <div className="flex items-center justify-between mb-4">
                  <h2 className="font-bold text-slate-900 dark:text-white">Smart Search</h2>
                  <button onClick={() => setMobileFilterOpen(false)} className="rounded-lg p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400">
                    <X className="h-5 w-5" />
                  </button>
                </div>
                <CvJobSearch />
